...
Particle::Particle() {
	attractPoints = NULL;
	changeVelocity = 1; //velocity multiplier
	increaseScale = 1; //scale multiplier
	particleShape = 0; // Change particle shape
}
...
void Particle::update() {
	...
	// add velocity decrease/increase
	pos += (changeVelocity * vel);
...
void Particle::draw() {
	...
	if (particleShape == 0) {
		ofDrawCircle(pos.x, pos.y, scale * 4.0 * increaseScale); // Added increaseScale variable
	}

	else if (particleShape == 1) { // Turns particle shape into triangles

		ofPoint p1(pos.x, pos.y - (scale * 4.0 * increaseScale));
		ofPoint p2(pos.x - (scale * 4.0 * increaseScale) * 0.87, pos.y + (scale * 4.0 * increaseScale) * 0.5);
		ofPoint p3(pos.x + (scale * 4.0 * increaseScale) * 0.87, pos.y + (scale * 4.0 * increaseScale) * 0.5);
...
void Particle::doubleVelocity() {
	changeVelocity *= 2; //sets velocity multiplier to 2
}
//------------------------------------------------------------------
void Particle::halfVelocity() {
	changeVelocity *= 0.5; //sets velocity multiplier to half
}
void Particle::normalVelocity() {
	changeVelocity = 1; //sets velocity multiplier back to normal
}
//------------------------------------------------------------------
void Particle::magnifyPartcle(ofRectangle &rect) {
	if (rect.inside(pos.x, pos.y)) { 
		// Once particles are inside the drawn circle they will magnify 5 times their original size
		increaseScale = 5;
	}
	else {
		// if particles exit the rectangle they return to normal size
		increaseScale = 1;
	}
}